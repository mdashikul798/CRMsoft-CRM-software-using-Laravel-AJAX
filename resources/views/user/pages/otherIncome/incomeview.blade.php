    ...
    <div id="wrapper">
      ...
      <article>
       ...
       @endforeach
        ...
        <address contentreadonly>
          <p>
            Name: {{ $allIncome['0']['customer_name'] }}<br>
            Phone: {{ $allIncome['0']['phone'] }}
          </p>
        </address>
        <table class="meta">
          <tr>
            <th><span contentreadonly>Invoice #</span></th>
            <td><span contentreadonly>{{ $allIncome['0']['invoiceNum'] }}</span></td>
          </tr>
          <tr>
        ...
        <table class="inventory">
          <thead>
            <tr>
              ...
              <th><span contentreadonly>Amount Due</span></th>
          ...
          <tbody>
            ...
            @foreach($allIncome as $income)
            @php
              $subtotal = $income->price - $income->due;
              $total_price += $subtotal;
              $amountDue += $income->due;
            @endphp
            <tr>
               <td>{{ $loop->index +1 }}</td>
               <td>{{ $income->income_name }}</td>
               <td>TK. {{ number_format($income->price) }}</td>
               <td>{{ $income->due }}</td>
        ...
        <table class="balance">
          ...
          <tr>
            ...
            <td>
              <p id="dueBalance">TK. {{ number_format($total_price - $amountDue - $discount) }}</p>
            </td>
          </tr>
          <tr>
      ...
      <aside>
        ...
        <a href="{{ route('other.income.print') }}" target="_blank" style="padding:10px;background:rgb(10 65 67 / 89%);color:#fff;float:right;"><img src="{{ asset('public/component') }}/assets/dist/img/printer.png" style="width:30px;height:24px;padding:0;margin-right:4px;margin-top:-3px;">Print</a>

        <a href="{{ route('other.income') }}" style="padding:10px;background:rgb(10 65 67 / 89%);color:#fff;float:right;margin-right:5px;"><img src="{{ asset('public/component') }}/assets/dist/img/home-icon.png" style="width:30px;height:24px;padding:0;margin-right:4px;margin-top:-3px;">Home</a>
      </aside>
      @else
